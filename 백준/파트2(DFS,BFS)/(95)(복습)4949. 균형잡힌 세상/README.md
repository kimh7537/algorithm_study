...
### 출력 
 ...
 ---
 **예외처리 확실히 하기**
ex. 23번째 줄의 `else{ flag = 1; break; }` 가 없다면 예외 처리 실패
-> `[)]`와 같은 경우가 들어오면 원래 `false`가 나와야 함
-> 위 코드가 없으면 `[`이 먼저 들어오고')'이 들어왔을때 예외코드가 없어서 아무일도 일어나지 않음
-> 다시 `]` 들어오면 자연스럽게 []이 맞게되어 스택에서 빠져버리고 `true`가 됨
