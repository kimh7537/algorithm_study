...
### 출력 
 ...
 ---
 ## 다시 생각할점
 - **`.size()`는 반환값이 `unsigned int` 이다. 따라서 `.size() - 3` 과 같은 연산을 하면 예상하지 못한 결과가 나올 수 있다.**
     -> 절대 연산하지 말고, 다른 부분에서 연산하기
- **문제에 조건이 3개가 나옴, 조건이 나올때는 flag=1과 같이 하나로 통일해주는게 문제를 풀때 편함**
